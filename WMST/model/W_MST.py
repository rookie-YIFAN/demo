...
class TDFE_G(nn.Module):
    def __init__(self, Patchsz, emb_dim, band, group, group_out):
        ...
        self.weightSmall = nn.Parameter(torch.tensor([0.4]))
        self.weightMid = nn.Parameter(torch.tensor([0.2]))
        self.weightLarge = nn.Parameter(torch.tensor([0.3]))

        self.weightSmall.data.clamp_(0.1, 1)
        self.weightMid.data.clamp_(0.1, 1)
        self.weightLarge.data.clamp_(0.1, 1)

        self.branchWeight = [self.weightSmall, self.weightMid, self.weightLarge]
    ...
    def forward(self, X):
        ...
        return self.weightLarge * x + self.weightSmall * residual + self.weightMid * middleScale
...
if __name__ == '__main__':
    ...
    DS = DEPTHWISECONV(80, 128, 8).cuda()
    summary(DS, ( 80, 13, 13))
    # summary(model, (1, 13, 13, 80))
